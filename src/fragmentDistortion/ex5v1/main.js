...
void main() {
    ...
    float l = length(delta) / u_time;

    float zoomFactor = (1. / (abs(sin(l * 1.)) * 1.)); // Adjust amplitude and frequency as needed

    // Use a periodic function (sin) to create a pulsating zoom effect
    if(
    (v_texCoord.x > zoomFactor && v_texCoord.y > zoomFactor)
    ) {
  ...
  `;
    ...
    const render = () => {
        ...
        gl.uniform1f(timeUniformLocation, performance.now() / 8000);

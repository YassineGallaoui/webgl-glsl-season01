...
const loadPageSCript = () => {
    ...
    shaders.vertexShader = `
        ...
        void main(){
            ...
            float radius = length(clipSpace);             // Distance from origin (radius)
            float angle = atan(clipSpace.y, clipSpace.x); // Angle in radians

            // Add time-based rotation to the angle (spiral effect)
            float rotationSpeed = 0.3;
            angle += u_time * rotationSpeed;

            // Optional: Expand or contract the spiral over time
            float expansion = sin(u_time) * 0.9 + 1.; // Oscillate between 0.5 and 1.5
            radius *= expansion;

            // Add a time-based distortion to the position
            clipSpace.x = radius * cos(angle);
            clipSpace.y = radius * sin(angle);
